...
<head>
    ...
    <!-- 1. AXIOS CDN (WAJIB ADA) -->
    <script src="https://cdn.jsdelivr.net/npm/axios/dist/axios.min.js"></script>
    
    <!-- 2. KONFIGURASI AXIOS -->
    <script>
        // Tunggu sampai DOM siap, lalu setting header
        document.addEventListener('DOMContentLoaded', function() {
            const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            if (window.axios) {
                window.axios.defaults.headers.common['X-CSRF-TOKEN'] = token;
                window.axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
                console.log('Axios CDN loaded & configured.'); // Debugging log
            }
        });
    </script>
...
<body class="bg-gray-100 font-sans antialiased h-dvh flex flex-col overflow-hidden">
    ...
    <!-- MAIN CONTENT -->
    <main class="flex-1 flex flex-col min-h-0 bg-gray-50 relative">
    ...
    <!-- FOOTER -->
    <div class="flex-none z-40 bg-gray-800">
        <x-footer/>
    </div>
        ...
        function chatApp(currentUserId, receiverId, initialMessages) {
            return {
                ...
                sendMessage() {
                    ...
                    // SOLUSI AXIOS: Cek apakah axios tersedia
                    if (typeof window.axios === 'undefined') {
                        alert('Error: Library sistem belum siap. Coba refresh halaman.');
